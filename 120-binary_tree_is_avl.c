...
/**
 * bal_avl - Auxiliar func to compare each subtree if its AVL.
 * @tree: node that point to the tree to check in prog.
 * @high: node that point to the higher node selected in prog
 * @lower: node that point to the lower node selected in prog.
 * Return: 1 if tree is AVL, 0 if not in prog.
 */
...
/**
 * binary_tree_is_avl - checks if a binary tree is a valid AVL tree in prog.
 * @tree: node that point to the tree to check in prog.
 * Return: 1 if tree is AVL, 0 if not in prog.
 */
