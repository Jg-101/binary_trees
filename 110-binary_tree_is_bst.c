 ...
 * the root specified in prog
 * @node: node in the tree to verify condition
 * @max: val to compare
 * Return: 1 if all nodes are smaller or equal or 0 if not in prog
 */
 ...
 * root specified in prog
 * @node: node in the tree to verify condition in prog
 * @min: val to compare in prog
 * Return: 1 if all is bigger or equal or 0 if not in prog
 */
...
/**
 * binary_tree_is_bst - says if a tree is a bst or not in prog
 ...
 * after that verify if the left subtree has nodes smaller than root in prog
 * and the right subtree has bigger nodes than root in prog
 * @tree: node that point to the tree to check in prog
 * Return: 1 if it is a BST or 0 if not in prog
 */
